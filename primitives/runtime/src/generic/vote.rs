...
impl PotVotes {
	...
	pub fn update_vote_weight(&mut self, vote_asset_id: VoteAssetId, vote_account_id: VoteAccountId, vote_weight: VoteWeight) {
	...
	pub fn votes(&self) -> PotVotesResult {
		let res = self.votes.clone()
				.into_iter()
				.map(|(k,v)| {
					PotVote::new(k.0, k.1, v)
				})
				.collect::<Vec<PotVote>>();
